...
namespace DocuEye.Structurizr.Json.Model.Maps.Tests
{
    public class StructurizrJsonModelMapTests
    {
        ...
        public void Mapping_StructurizrJsonModel_To_LinterModelElement_Works()
        {
            ...
            var source = new StructurizrJsonModel
            {
                ...
                People = new[]
                {
                    new StructurizrJsonPerson
                    {
                        ...
                        Name = "Person 1",
                        Properties = new Dictionary<string, string>() {
                           { DslPropertyNames.DslIdProperty, "person1-dsl-id" },
                        },
                        Tags = "Tag7,Tag8"
                    }
                },
            ...
            Assert.That(elements.Any(e => e.Identifier == "person1-dsl-id"));
        ...
        [Test]
        public void Mapping_StructurizrJsonModel_To_LinterModelRelationships_Works()
        {
            // Arrange
            var source = new StructurizrJsonModel
            {
                    ...
                    new StructurizrJsonPerson
                    {
                        ...
                        Relationships = new[]
                        {
                            new StructurizrJsonRelationship
                            {
                                ...
                                Properties = new Dictionary<string, string>() {
                                   { DslPropertyNames.DslIdProperty, "r1-dsl-id" },
                                },
                            },
                            new StructurizrJsonRelationship
                            {
                                ...
                                Properties = new Dictionary<string, string>() {
                                   { DslPropertyNames.DslIdProperty, "r2-dsl-id" },
                                },
                            }
                        }
                    }
            ...
            // Act
            var elements = source.ToLinterModelElements();
            var relationships = source.ToLinterModelRelationships(elements);
            // Assert
            Assert.That(relationships.Count(), Is.EqualTo(2));
            Assert.That(relationships.Any(e => e.Identifier == "r1-dsl-id"));
            Assert.That(relationships.Any(e => e.Identifier == "r2-dsl-id"));

            Assert.That(relationships.Where(r => r.Identifier == "r1-dsl-id").First().Source.Identifier, 
                Is.EqualTo("person1-dsl-id"));
            Assert.That(relationships.Where(r => r.Identifier == "r1-dsl-id").First().Destination.Identifier,
                Is.EqualTo("c1-dsl-id"));
            Assert.That(relationships.Where(r => r.Identifier == "r2-dsl-id").First().Source.Identifier,
                Is.EqualTo("person1-dsl-id"));
            Assert.That(relationships.Where(r => r.Identifier == "r2-dsl-id").First().Destination.Identifier,
                Is.EqualTo("ss1-dsl-id"));
