...
{
    public class ConvertComponentViewTests
    {
        private readonly StructurizrWorkspace workspace = new StructurizrWorkspace()
        {
            Model = new StructurizrModel()
            {
                Elements = new List<StructurizrModelElement>()
                {
                    ...
                    new StructurizrModelElement("c0","component1")
                    {
                        Name = "Component1",
                        ParentIdentifier = "container1",
                ...
                Relationships = new List<StructurizrRelationship>()
                {
                    ...
                    new StructurizrRelationship("id3", new string[] { "tag1" })
                    {
                        ModelId = "4",
                        SourceIdentifier = "person0",
                        DestinationIdentifier = "component1",
                    },
                }
            }
        ...
        public void WhenViewIsConvertedThenValidJsonIsReturned()
        {
            ...
            var view = new StructurizrComponentView("container1", "Components0", "Test Description")
            {
                Title = "Test Title",
                Include = new string[] { "*" }
            };
            // Act
            var result = workspaceConverter.ConvertComponentView(view);
            // Assert
            Assert.That(result, Is.Not.Null);
            Assert.That(result.ContainerId, Is.EqualTo("container1"));
            Assert.That(result.Key, Is.EqualTo("Components0"));
