...
{
    public class VisitCustomViewTests : BaseTests
    {
        [Test]
        public void WhenDefineCustomViewThenAllPropertiesAreSet()
        {
            // Arrange
            var parser = CreateParserFromText(@"custom viewId ""Great system title"" ""view description"" 
            {
            ...
            var context = parser.customView();
            var visitor = new DSLVisitor();
            // Act
            var result = visitor.VisitCustomView(context);
            // Assert
