...
{
    public class VisitSystemContextTests : BaseTests
    {
        [Test]
        public void WhenDefineSystemContextThenAllPropertiesAreSet()
        {
            // Arrange
            var parser = CreateParserFromText(@"systemContext systemId viewId ""view description"" 
            {
            ...
            var context = parser.systemContext();
            var visitor = new DSLVisitor();
            // Act
            var result = visitor.VisitSystemContext(context);
            // Assert
