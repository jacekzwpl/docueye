...
{
    public class VisitComponentViewTests : BaseTests
    {
        [Test]
        public void WhenDefineComponentViewThenAllPropertiesAreSet()
        {
            // Arrange
            var parser = CreateParserFromText(@"component containerId viewId ""view description"" 
            {
            ...
            var context = parser.componentView();
            var visitor = new DSLVisitor();
            // Act
            var result = visitor.VisitComponentView(context);
            // Assert
            Assert.That(result.ElementIdentifier, Is.Not.Null);
            Assert.That(result.ElementIdentifier, Is.EqualTo("containerId"));
